...
async def send_config_ev(bot: Bot, ev: Event):
    logger.info('[星铁] 开始执行[设置阈值信息]')
    uid = await GsBind.get_uid_by_game(ev.user_id, ev.bot_id, 'sr')

    if uid is None:
        return await bot.send(UID_HINT)
    cookie = await GsUser.get_user_cookie_by_uid(uid, 'sr')
    if cookie is None:
...
async def open_switch_func(bot: Bot, ev: Event):
    ...
    logger.info(
        f'[星铁服务] [{user_id}]尝试[{ev.command[2:]}]了[{ev.text}]功能'
    )
    ...
    if '开启' in ev.command:
        ...
        if PRIV_MAP[config_name]:
            ...
            if not await gs_subscribe.get_subscribe('[星铁] 推送', uid=uid):
                im += '\n⚠ 警告: 由于未打开推送总开关, 所以此项设置可能无效！'
                im += f'如需打开总开关, 请发送命令开启推送: {P}开启推送！'
        if config_name == '推送':
            resin = await gs_subscribe.get_subscribe('[星铁] 体力', uid=uid)
            go = await gs_subscribe.get_subscribe('[星铁] 派遣', uid=uid)

            im += f'\n✅ 如需关闭请发送命令: {P}关闭推送\n💚 该项为总开关, 你开可以单独开启体力、派遣的推送。'
            im += '\n🔖 【当前推送设置状态】'
            if resin:
